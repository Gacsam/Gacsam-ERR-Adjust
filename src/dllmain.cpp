...
{
    auto logger = std::make_shared<spdlog::logger>("gacsam");
...
static void setup_mod()
{
    ...
    spdlog::info("Running Gaccy adjustments...");
    gacsam::Initialise();
...
bool WINAPI DllMain(HINSTANCE dll_instance, unsigned int fdw_reason, void *lpv_reserved)
{
    if (fdw_reason == DLL_PROCESS_ATTACH)
    {
        ...
        setup_logger(folder / "logs" / "Gacsam.log");

#ifdef PROJECT_VERSION
        spdlog::info("Gacsam's Adjustments version {}", PROJECT_VERSION);
#endif
